...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    res = authorized_client.post("/posts/", json={"title": "something", "content": "something something"})
    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
...
def test_unauthorized_user_get_create_post(client,test_posts):
    res = client.post("/posts/", json={"title": "something", "content": "something something"})
    assert res.status_code == 401  
...
def test_delete_post_nonexist(authorized_client, test_user, test_posts):
    res = authorized_client.delete(f"/posts/8989")
    assert res.status_code == 404
